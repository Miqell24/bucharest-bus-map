...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: TPBI GTFS — STB · Metrorex · Ilfov operators' }));
  ...
  document.getElementById('chips').innerHTML = meta.lines
    .map((l) => `<button class="chip${l.h24 ? ' h24' : ''}" data-line="${esc(l.line)}" style="background:${esc(l.color)}">${esc(l.line)}${l.h24 ? '<span class="plus">+</span>' : ''}</button>`)
    .join(' ');
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: TPBI (STB, Metrorex, Ilfov operators)';
      ...
      a.download = `bucharest-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `bucharest-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // core. Open on central Bucharest instead.
  map.jumpTo({ center: [26.10, 44.435], zoom: 11.0 });
}
