...
// Bucharest & Ilfov: ONE TPBI feed (gtfs.tpbi.ro/regional/) split by route_type
// into two modes — buses+trolleybuses (3, 11) on the road graph and trams+metro
// (0, 1) on the rail graph. Trolleybuses ride the bus network in green per-line
// colors; the Metrorex lines M1–M5 keep their official colors straight from
...
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|1,M2]
...
// ONE feed, TWO modes: routeTypes splits the TPBI bundle per mode (3 = bus,
// 11 = trolleybus riding the bus network in green; 0 = tram, 1 = Metrorex
// metro riding the rail mode in its official line colors). Without the filter
// `--all` on the bus mode would swallow the rail lines too.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/bucharest.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['100']),
  feeds: [
    { tag: 'bi', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['3', '11'] },
  ],
}];
...
const tramSel = tramLines.filter((l) => !l.startsWith('M'));
const metroSel = tramLines.filter((l) => l.startsWith('M'));
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/bucharest-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'bi', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['0'] },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  mode: 'tram', label: 'metro', osmFile: 'data/osm/bucharest-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'bi', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['1'] },
  ],
});
...
async function processMode(cfg) {
  ...
  const isMetroMix = (lines) => cfg.mode === 'tram' && lines.length > 1 && lines.every((l) => l.startsWith('M'));
  const colorOf = (lines) => {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      if (r.route_type === '11') {
        ...
        // Metrorex ships the official line colors (M1 yellow … M5 orange)
  ...
  for (const r of reps) {
    ...
    if (r.pseudo) {
      ...
      if (cfg.mode === 'tram' && r.line.startsWith('M')) {
